const createCarEntry = (db:any, carDetails:any) => {
    return new Promise((resolve, reject) => {
        ...
        db.get("SELECT reg FROM cars WHERE vin = ?", [carDetails.vin], (err:any, row:any) => {
            ...
            const query = `
                INSERT INTO cars (reg, vin, engine_number, transmission_number, date_of_manufacture, date_of_registration, colour, paint_code, warranty_start_date, warranty_end_date, recalls, make, model, manufacturer, manufacturer_location, country_of_origin, number_of_axels, weight, fuel_type, drive_type)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
            `;
